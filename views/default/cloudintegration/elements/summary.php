...
if ($entity instanceof ElggEntity) {
	$vars['title'] = elgg_view('output/url', [
		...
		'href' =>  $entity->link, //$entity->getURL(),
	]);
}
...
if ($entity instanceof ElggEntity) {
	...
	$original = elgg_view('output/url', [
		'text' => elgg_echo('cloudintegration:share:view:dc'),
		'href' => $entity->getURL(), 
	]);
	
	$plus = elgg_view('object/elements/summary/subtitle', [
		'subtitle' => $original,
	]);
}
...
$summary = $metadata . $title . $plus . $subtitle . $tags . $extensions . $content;
